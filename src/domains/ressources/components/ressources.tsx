...
const Ressources = ({ listeRessources }: { listeRessources: Ressource[] }) => {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 h-screen">
      {listeRessources && listeRessources.length > 0 ? (
        listeRessources?.map((ressource) => (
          <a
            ...
            className={`group relative flex  w-28 md:w-44 lg:w-64 rounded-lg  flex-col justify-between`}
          >
            ...
            <div className="absolute pr-2 pl-2 inset-0 flex text-center items-center justify-center text-lg font-bold text-white">
              {ressource.name}
            </div>
          </a>
        ))
