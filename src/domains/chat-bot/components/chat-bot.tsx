...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState<{ role: string; message: string }[]>(
    [{ role: 'assistant', message: t('placeholder') }]
  );
  ...
  return (
    <div
      ...
      className={clsx(
        'fixed bottom-10 z-20 right-10 w-20 h-20 text-black hidden md:block  bg-white shadow-lg transition-all duration-300 ease-in-out',
        isChatBotEnabled
          ? 'rounded-lg h-96 w-90'
          : ' rounded-full w-20 h-20 hover:cursor-pointer'
      )}
    >
      {isChatBotEnabled && isAnimationComplete ? (
        <div className="relative flex overflow-hidden flex-col items-center justify-between h-full w-full p-4 gap-1">
          <div
            ...
            className="absolute top-2 z-30  right-2 flex flex-col gap-2 items-center justify-center hover:cursor-pointer bg-red-400 w-8 h-8 rounded-full"
          >
            X
          </div>
          <div className="w-full h-full pt-10 flex flex-col gap-2 overflow-y-auto scroll-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
            {messages.map(
              (message: { role: string; message: string }, index: number) => (
                <div
                  key={index}
                  className={clsx(
                    message.role === 'assistant'
                      ? 'self-start text-sm bg-amber-300 px-2 py-1 rounded-lg text-left'
                      : 'w-fit h-auto self-end text-sm bg-green-300 px-2 py-1 rounded-lg text-left'
                  )}
                >
                  {message.role === 'assistant' ? (
                    <div
            ...
            {isLoading && (
              <div className="w-fit h-auto self-start items-end text-sm px-2 py-1 rounded-xl text-left">
                <LoadingSpinner />
              </div>
            )}
          ...
          <div className="w-full">
            ...
            >
              <Input
                placeholder={
                  error?.message ? error?.message : t('placeholderMessage')
                }
                className="w-full"
              ...
              <button
                ...
                disabled={isLoading}
                className={clsx(
                  'w-10 h-10 hover:cursor-pointer disabled:opacity-50',
