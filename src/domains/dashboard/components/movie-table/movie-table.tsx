'use client'
import { Table, TableBody, TableHead, TableHeader, TableRow } from '@/domains/ui/components/table/table'
      ...
      <Table >
          ...
          <TableBody>
            {filteredMoviesAdded ? filteredMoviesAdded?.map((movie, index) => (
              <MovieRow key={movie?.id} movie={movie}  index={index + 1} btnText={'Editer'} />
            )): t('noMovie')}
